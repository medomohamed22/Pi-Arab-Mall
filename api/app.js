...
async function assertProductParticipant(productId, user, otherId) {
  ...
  if (product.seller_pi_id !== user.pi_id && product.seller_pi_id !== otherId) throw new Error('Invalid chat target');
  return product;
}

async function handlePublic(action, body) {
...
async function handleAuthed(action, body, req) {
  if (action === 'login') {
  ...
  if (action === 'messages.inbox') {
    ...
    return { messages: data || [], users: Object.fromEntries((users || []).map(u => [u.pi_id, u.username])) };
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const publicResult = await handlePublic(body.action, body);
    const result = publicResult || await handleAuthed(body.action, body, req);
