...
async function adminApi(action,payload={}){const res=await fetch('/api/admin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,adminPiId:adminUser?.uid,...payload})});const json=await res.json().catch(()=>({}));if(!res.ok)throw new Error(json.error||'Admin API failed');return json}

async function loadAdminData(){
  try{const data=await adminApi('list');allProducts=data.products||[];allUsers=data.users||[];allReports=data.reports||[];renderStats();renderAllTabs();return}catch(apiError){console.warn('Admin API fallback:',apiError);toast(apiError.message.includes('SUPABASE_SERVICE_ROLE_KEY')?'اضبط SUPABASE_SERVICE_ROLE_KEY و ADMIN_PI_IDS في Vercel':'تعذر استخدام API الأدمن، سيتم التجريب بالمفتاح العام')}
...
async function setProductStatus(id,status){try{await adminApi('productStatus',{productId:id,status})}catch(e){const {error}=await sb.from('products').update({status,reviewed_by:adminUser.uid,reviewed_at:new Date().toISOString()}).eq('id',id);if(error&&/reviewed_by|reviewed_at|schema cache|column/i.test(error.message||'')){const fallback=await sb.from('products').update({status}).eq('id',id);if(fallback.error){toast(fallback.error.message);return}}else if(error){toast(error.message);return}}toast(status==='active'?'تم نشر الإعلان':'تم رفض الإعلان');await loadAdminData()}
async function deleteProductAdmin(id){if(!confirm('حذف الإعلان نهائيًا؟'))return;try{await adminApi('deleteProduct',{productId:id})}catch(e){const {error}=await sb.from('products').delete().eq('id',id);if(error){toast(error.message);return}}toast('تم حذف الإعلان');await loadAdminData()}
...
async function setUserBan(piId,isBanned){try{await adminApi('setUserBan',{piId,isBanned})}catch(e){const {error}=await sb.from('users').update({is_banned:isBanned}).eq('pi_id',piId);if(error){toast(error.message);return}}toast(isBanned?'تم حظر الحساب':'تم فك الحظر');await loadAdminData()}
...
async function setReportStatus(id,status){try{await adminApi('reportStatus',{reportId:id,status})}catch(e){const {error}=await sb.from('reports').update({status}).eq('id',id);if(error){toast(error.message);return}}toast('تم تحديث البلاغ');await loadAdminData()}
