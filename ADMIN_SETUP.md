...
function showAdminError(msg) { const box = el('adminError'); if (!box) return toast(msg); box.innerHTML = escapeHtml(msg).replace(/SUPABASE_SERVICE_ROLE_KEY|admins|products/g, '<code>$&</code>'); box.classList.remove('hidden'); }
function clearAdminError() { el('adminError')?.classList.add('hidden'); }
...
function setLoading(isLoading) { const btn = el('adminLoginBtn'); if (!btn) return; btn.disabled = isLoading; btn.innerHTML = isLoading ? '<i class="fa-solid fa-circle-notch fa-spin"></i> جاري الدخول' : '<i class="fa-solid fa-lock"></i> دخول'; }
...
async function init() {
  const ready = await waitForSupabaseSdk();
  if (!ready) return showAdminError('تعذر تحميل مكتبة Supabase. تأكد من اتصال الإنترنت أو رابط المكتبة.');
  sb = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  el('adminLoginBtn').onclick = loginAdmin;
  el('adminLogoutBtn').onclick = logoutAdmin;
  const { data } = await sb.auth.getSession();
  if (data?.session) {
    adminSession = data.session;
    try { await verifyAndEnterDashboard(); } catch (e) { await logoutAdmin(false); showAdminError(e.message || 'جلسة الأدمن غير صالحة'); }
  }
}
...
async function loginAdmin() {
  ...
  if (!email || !password) return showAdminError('اكتب البريد وكلمة المرور.');
  setLoading(true);
  try {
    const { data, error } = await sb.auth.signInWithPassword({ email, password });
    if (error) throw error;
    adminSession = data.session;
    await verifyAndEnterDashboard();
  } catch (e) {
    console.error(e);
    await sb.auth.signOut().catch(() => {});
    adminSession = null;
    showAdminError(adminFriendlyError(e));
  } finally {
    setLoading(false);
  }
}

async function logoutAdmin(showLogin = true) {
  await sb?.auth?.signOut?.().catch(() => {});
  adminSession = null;
  allProducts = []; allUsers = []; allReports = [];
  el('dashboard').classList.add('hidden');
  if (showLogin) el('accessPanel').classList.remove('hidden');
  el('adminLogoutBtn').classList.add('hidden');
}
...
async function verifyAndEnterDashboard() {
  ...
  el('adminLogoutBtn').classList.remove('hidden');
  toast(`تم الدخول: ${me.email}`);
  await loadAdminData();
}

function adminFriendlyError(e) {
  const msg = String(e?.message || e || 'فشل تسجيل الدخول');
  if (msg.includes('Invalid login credentials')) return 'البريد أو كلمة المرور غير صحيحة في Supabase Auth.';
  if (msg.includes('SUPABASE_SERVICE_ROLE_KEY')) return 'متغير SUPABASE_SERVICE_ROLE_KEY غير موجود في Vercel. أضفه من Environment Variables ثم أعد النشر.';
  if (msg.includes('ليس مضافًا كأدمن')) return msg + ' - أضف نفس البريد بالضبط في جدول admins.';
  if (msg.includes('Invalid admin session')) return 'جلسة الدخول غير صالحة. سجل الدخول مرة أخرى.';
  return msg;
}

async function adminApi(action, payload = {}) {
  const token = adminSession?.access_token;
  if (!token) throw new Error('جلسة الأدمن غير موجودة');
...
async function setProductStatus(id, status) { try { await adminApi('productStatus', { productId: id, status }); toast(status === 'active' ? 'تم نشر الإعلان' : 'تم رفض الإعلان'); await loadAdminData(); } catch (e) { showAdminError(adminFriendlyError(e)); } }
async function deleteProductAdmin(id) { if (!confirm('حذف الإعلان نهائيًا؟')) return; try { await adminApi('deleteProduct', { productId: id }); toast('تم حذف الإعلان'); await loadAdminData(); } catch (e) { showAdminError(adminFriendlyError(e)); } }
...
async function setUserBan(piId, isBanned) { try { await adminApi('setUserBan', { piId, isBanned }); toast(isBanned ? 'تم حظر الحساب' : 'تم فك الحظر'); await loadAdminData(); } catch (e) { showAdminError(adminFriendlyError(e)); } }
...
async function setReportStatus(id, status) { try { await adminApi('reportStatus', { reportId: id, status }); toast('تم تحديث البلاغ'); await loadAdminData(); } catch (e) { showAdminError(adminFriendlyError(e)); } }
